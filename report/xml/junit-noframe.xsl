...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
  ...
  <xsl:param name="TITLE">SLA UI Cucumber Report (Continuous Compliance &amp; Self Service Delivery)</xsl:param>
  <xsl:param name="LOG"><xsl:value-of select="testsuites/@logs"/></xsl:param>
  <xsl:param name="REPORT_TIME"><xsl:value-of select="testsuites/@timestamp"/></xsl:param>
  <!--
    ...
    <html>
     ...
     function displayProperties (name) {
      ...
      <body>
        ...
        <a id="time">Report generation time: <xsl:value-of select="$REPORT_TIME"/></a><!-- p id="time"></p -->
        <!-- script>
  ...
  <xsl:template name="packagelist">
    ...
    <table class="details" border="0" cellpadding="5" cellspacing="2" width="95%">
      ...
      <xsl:for-each select="./testsuite[not(./@package = preceding-sibling::testsuite/@package)]">
        ...
        <xsl:variable name="build" select="$testsuites-in-package/@build"/>
        <xsl:variable name="gitBranch" select="$testsuites-in-package/@git_branch"/>
        <xsl:variable name="sprint" select="$testsuites-in-package/@sprint"/>
        ...
        <tr valign="top">
          ...
          <td><xsl:value-of select="$gitBranch"/></td>
          <td><xsl:value-of select="$sprint"/></td>
  ...
  <xsl:template name="display-time">
    ...
    <xsl:choose>
      <xsl:when test="$value &gt;= 3600">
        <xsl:value-of select="concat(format-number(floor($value div 3600), '00'), ':')"/>
      </xsl:when>
