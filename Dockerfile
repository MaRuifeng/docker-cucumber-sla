# Dockerfile for cucumber test environment setup - SLA UI
#
# VERSION 0.1
...
FROM gts-tia-sla-docker-local.artifactory.swg-devops.com/gts-docker-library/ubuntu:14.04
# FROM ubuntu:14.04
...
# Install Chef Development Kit for the Chef helper to support test automation
RUN curl https://omnitruck.chef.io/install.sh | sudo bash -s -- -P chefdk -c stable -v 0.18.30
...
# Start Xvfb, x11vnc, ssh services and run cucumber and report utils (using CMD shell form to parse the env vars)
CMD /bin/bash
